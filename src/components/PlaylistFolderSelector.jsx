...
const PlaylistFolderSelector = ({ onSelect, onCancel }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-[60] p-4">
      <div className="bg-white rounded-lg border border-slate-200 w-full max-w-2xl max-h-[80vh] flex flex-col">
        {/* Header */}
        ...
        <div className="flex-1 overflow-y-auto p-4">
          {loading ? (
            ...
            <div className="space-y-2">
              {playlists.length === 0 ? (
                <div className="text-slate-500 text-center py-8">No playlists found</div>
              ) : (
                playlists.map(playlist => {
                  ...
                  return (
                    <div key={playlist.id} className="border border-slate-300 rounded-lg overflow-hidden">
                      {/* Playlist Header */}
                      <div className="flex items-center">
                        ...
                        >
                          <div className="flex items-center justify-between">
                            <div className="flex-1">
                              <div className="text-slate-900 font-medium">{playlist.name}</div>
                              {playlist.description && (
                                <div className="text-slate-500 text-sm mt-1 line-clamp-1">
                                  {playlist.description}
                                </div>
                              )}
                              {hasFolders && (
                        ...
                        {hasFolders && (
                          <button
                            ...
                            className="px-3 py-3 bg-slate-100 hover:bg-slate-200 transition-colors border-l border-slate-300"
                          ...
                          {playlistFolders.map((folder, index) => {
                            ...
                            return (
                              <button
                                ...
                                className={`w-full text-left p-3 border-b border-slate-200 last:border-b-0 transition-all ${
        ...
        <div className="flex items-center justify-end gap-3 p-4 border-t border-slate-200">
          <button
            onClick={onCancel}
            className="px-4 py-2 bg-slate-100 text-slate-900 rounded-lg hover:bg-slate-200 transition-colors"
          >
            Cancel
          </button>
          <button
