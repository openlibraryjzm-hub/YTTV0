...
- **History Page Display**: Users see watch history in History page (see Section 4.1.3):
  ...
  - **Playlist/Folder Badges**: Each history item displays badges showing:
    - Which playlists the video belongs to
    - Which folders (if any) the video is assigned to in each playlist
    - Badge format: "Playlist Name - Folder Name" (or just "Playlist Name" if no folder)
    - Badge colors match the folder color (or default sky if no folder)
    - **Interactive Navigation**: Badges have two clickable parts:
      - Clicking playlist name navigates to that playlist (preview mode)
      - Clicking folder name navigates to that specific folder (preview mode)
    - Hover effects show highlight boxes around each clickable part
